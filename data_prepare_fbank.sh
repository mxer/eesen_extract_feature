...
if [ ${stage} -le 2 ]; then
  ...
  for set in train test; do
    steps/make_fbank.sh --cmd "$train_cmd" --nj 20 data/${set} exp/make_fbank/${set} ${fbank_dir} || exit 21;
    utils/fix_data_dir.sh data/${set} || exit 22;
    steps/compute_cmvn_stats.sh data/${set} exp/make_fbank/${set} ${fbank_dir} || exit 23;
  done
