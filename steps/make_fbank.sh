...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_fbank_${name}.JOB.log \
    ...
    compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config ark:- ark,scp,t:$fbankdir/raw_fbank_$name.JOB.ark,$fbankdir/raw_fbank_$name.JOB.scp || exit 208;
...
#     || exit 208;
  ...
  $cmd JOB=1:$nj $logdir/make_fbank_${name}.JOB.log \
    compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config scp,p:$logdir/wav.JOB.scp ark,scp,t:$fbankdir/raw_fbank_$name.JOB.ark,$fbankdir/raw_fbank_$name.JOB.scp  || exit 209;
#    copy-feats --compress=$compress ark:- \
#     ark,scp:$fbankdir/raw_fbank_$name.JOB.ark,$fbankdir/raw_fbank_$name.JOB.scp \
#     || exit 209;
