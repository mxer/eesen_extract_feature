...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ...
    compute-mfcc-feats $vtln_opts --verbose=2 --config=$mfcc_config ark:- ark,scp,t:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp || exit 208;

#    compute-mfcc-feats $vtln_opts --verbose=2 --config=$mfcc_config ark:- ark:- \| \
#      copy-feats --compress=$compress $write_num_frames_opt ark:- \
#      ark,scp:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
#      || exit 1;
#    compute-mfcc-feats $vtln_opts --verbose=2 --config=$mfcc_config ark:- ark,scp,t:$fbankdir/raw_mfcc_$name.JOB.ark,$fbankdir/raw_mfcc_$name.JOB.scp || exit 208;
#    compute-mfcc-feats $vtln_opts --verbose=2 --config=$mfcc_config ark:- ark,scp,t:- || exit 208;
#    copy-feats --compress=$compress ark:- \
#      ark,scp:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
#     || exit 1;

else
  echo "$0: [info]: no segments file exists " || exit 1;
fi
